...
class Sentence():
    ...
    def known_safes(self):
        ...
        return self.cells if not self.count else None
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for row in range(n-1, n+2):
            for col in range(m-1, m+2):
                rc = (row, col)
                if rc in cell: continue
                if 0 <= row < self.height and 0 <= col < self.width:
                    if rc not in self.moves_made and rc not in self.mines:
                        new_cells.add(rc)
                    elif rc in self.mines:
                        count -= 1
        ...
        for s in self.knowledge:
            ...
            if safe:
                for cell in copy.deepcopy(safe):
                    self.mark_safe(cell)
            if mine:
                for cell in copy.deepcopy(mine):
                    self.mark_mine(cell)
            ...
            for s2 in self.knowledge:
                ...
                elif s.cells.issubset(s2.cells):
                    new_sentence = Sentence(s2.cells-s.cells, s2.count-s.count)
    ...
    def make_safe_move(self):
        ...
        return list(safe_move)[0] if safe_move else None
